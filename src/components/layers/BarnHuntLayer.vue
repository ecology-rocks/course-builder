...
// --- REFS FOR DIRECT NODE ACCESS ---
// We store the Konva node references here so we can move them efficiently
const baleRefs = ref({}) 

// --- STATE FOR GROUP DRAGGING ---
...
function handleLeftClick(e, id) {
  if (store.activeTool === 'hide') {
    ...
    store.addHide((p.x - props.GRID_OFFSET)/props.scale, (p.y - props.GRID_OFFSET)/props.scale)
    return
  }
  ...
  if (store.activeTool === 'select' || store.activeTool === 'bale') {
    ...
    if (!store.selection.includes(id) && !isMulti) {
       store.selectBale(id, false) 
    } else if (isMulti) {
       store.selectBale(id, true) 
    }
    return
  }
...
// === GROUP DRAGGING LOGIC (Fixed) ===

function handleDragStart(e, id) {
  // 1. Auto-select if dragging an unselected item
  if (!store.selection.includes(id)) {
    store.selectBale(id, false)
  }

  // 2. Snapshot positions of ALL selected items
  dragStartPos.value = {}
  store.selection.forEach(selId => {
    // Look up the node in our baleRefs map (Safe & Fast)
    const node = baleRefs.value[selId]?.getNode()
    if (node) {
      dragStartPos.value[selId] = { x: node.x(), y: node.y() }
    }
  })
}
...
function handleDragMove(e, id) {
  ...
  // 1. Calculate Delta (Pixels) from Start
  ...
  // 2. Apply Delta to all PEERS
  store.selection.forEach(selId => {
    if (selId === id) return // Leader is handled by Konva
...
function handleDragEnd(e, id, type) {
  ...
  // Calculate final grid position of the LEADER
  const rawX = (absPos.x - layerAbs.x) / props.scale
  const rawY = (absPos.y - layerAbs.y) / props.scale

  if (type === 'bale') {
    // Adjust for offset center -> top-left
    const finalX = rawX - 1.5
    const finalY = rawY - 0.75
    
    // Use nextTick to ensure the drag event clears before we update state
    nextTick(() => {
      store.commitDrag(id, finalX, finalY)
    })
  } 
  else if (type === 'hide') {
...
// ... Board/Mat/Hide Handlers (Unchanged) ...
  ...
  <v-group>
    ...
    <v-group v-for="bale in visibleBales" :key="bale.id"
      ...
      :config="{ 
        ...
        /* CHANGE HERE: 
           Allow interaction if it's on the active layer 
           OR if it's part of the current selection.
        */
