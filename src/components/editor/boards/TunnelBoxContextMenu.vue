...
const menuState = computed(() => store.activeTunnelBoxMenu) // change menu here
...
function deleteBoard() {
  store.removeTunnelBoard(props.id)
  emit('close')
}

function rotateBoard() {
  store.rotateTunnelBoard(props.id)
  emit('close')
}
...
<template>
  <div class="context-menu" :style="style" ref="menuRef">
    ...
    <div class="action-stack">
      ...
      <button @click="deleteBoard" class="action-btn delete">🗑️ Delete Board</button>
    </div>
  </div>
</template>
