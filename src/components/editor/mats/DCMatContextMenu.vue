...
const menuState = computed(() => store.activeDCMatMenu) // change menu here
...
function deleteMat() {
  store.removeDCMat(props.matId)
  emit('close')
}

function rotateMat() {
  store.rotateDCMat(props.matId)
  emit('close')
}

function openCustomizer() {
  store.editingCustomObject = props.matId
...
<template>
  <div class="context-menu"  :style="style" ref="menuRef">
    <div class="menu-header">Mat Options</div>
    
    <div class="action-stack">
      <button @click="openCustomizer" class="action-btn customize">🎨 Customize Style</button>
      <button @click="rotateMat" class="action-btn rotate">🔄 Rotate Mat</button>
      <button @click="deleteMat" class="action-btn delete">🗑️ Delete Mat</button>
